...
final class ThumbnailPickerCoordinator: NSObject, PHPickerViewControllerDelegate {
    ...
    // Scales to max 1200 px on the long edge and encodes at 0.75 quality (~200–400 KB typical).
        ...
        for result in results {
            ...
            if provider.canLoadObject(ofClass: UIImage.self) {
                provider.loadObject(ofClass: UIImage.self) { imageObject, error in
                    ...
                    guard let jpegData = ThumbnailPickerCoordinator.compressForUpload(image) else {
                        return
                    }
            ...
            provider.loadFileRepresentation(forTypeIdentifier: imageType) { url, error in
                defer { group.leave() }
                // Load as UIImage so HEIC/HEIF is decoded and we can compress consistently
                guard let sourceURL = url, error == nil,
                      let image = UIImage(contentsOfFile: sourceURL.path),
                      let jpegData = ThumbnailPickerCoordinator.compressForUpload(image) else {
                    return
                }
