...
module.exports = {
  ...
  // Falls back to a static code whenever no Resend key is configured (any env), since
  // no real OTP email can be sent in that case anyway. Self-disables the moment
  // RESEND_API_KEY is set, so no separate revert step is needed once email is wired up.
  signupOtpTestCode: process.env.SIGNUP_OTP_TEST_CODE || (process.env.RESEND_API_KEY ? '' : '123456'),
