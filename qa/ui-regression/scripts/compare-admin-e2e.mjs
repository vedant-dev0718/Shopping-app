...
const baselineDir = path.join(rootDir, "qa", "ui-regression", "screenshots", "baseline", "admin-e2e");
const currentDir = path.join(rootDir, "qa", "ui-regression", "screenshots", "current", "admin-e2e");
const stitchDir = path.join(rootDir, "qa", "ui-regression", "screenshots", "current", "stitch-admin-e2e");
const diffDir = path.join(rootDir, "qa", "ui-regression", "diff", "admin-e2e");
...
const checkpoints = ["auth_login", "console"];
const stitchCheckpoints = new Set(["auth_login"]);
const pairs = [];
for (const checkpoint of checkpoints) {
    pairs.push({
        name: `admin_e2e_${checkpoint}_app_vs_baseline`,
        expected: path.join(baselineDir, `${checkpoint}.png`),
        actual: path.join(currentDir, `${checkpoint}.png`),
        gate: "hard",
    });
    if (stitchCheckpoints.has(checkpoint)) {
        pairs.push({
            name: `admin_e2e_${checkpoint}_app_vs_stitch`,
...
function shouldCropSystemBars(inputPath) {
    return inputPath.includes(`${path.sep}screenshots${path.sep}baseline${path.sep}admin-e2e${path.sep}`)
        || inputPath.includes(`${path.sep}screenshots${path.sep}current${path.sep}admin-e2e${path.sep}`);
}
...
async function loadPngNormalized(inputPath, width, height) {
    ...
    if (shouldCropSystemBars(inputPath)) {
        ...
        const cropTop = Math.min(appCropTopPx, Math.max(srcHeight - 1, 0));
        const cropBottom = Math.min(appCropBottomPx, Math.max(srcHeight - cropTop - 1, 0));
...
console.log(`[done] admin e2e comparison summary: ${summaryPath}`);

if (hardGateFailed) {
    console.error(`[fail] Admin e2e hard gate breached. Baseline mismatch exceeded ${hardGatePct}% on at least one checkpoint.`);
    process.exit(2);
}

if (stitchGateFailed) {
    console.error(`[fail] Admin e2e stitch gate breached. Stitch mismatch exceeded ${stitchGatePct}% on at least one checkpoint.`);
    process.exit(3);
}
