...
console.log(`[done] buyer e2e comparison summary: ${summaryPath}`);

if (hardGateFailed) {
    console.error(`[fail] Buyer e2e hard gate breached. Baseline mismatch exceeded ${hardGatePct}% on at least one checkpoint.`);
    process.exit(2);
}

if (stitchGateFailed) {
    console.error(`[fail] Buyer e2e stitch gate breached. Stitch mismatch exceeded ${stitchGatePct}% on at least one checkpoint.`);
    process.exit(3);
}
