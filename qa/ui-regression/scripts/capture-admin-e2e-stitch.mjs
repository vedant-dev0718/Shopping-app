...
const outDir = path.join(rootDir, "qa", "ui-regression", "screenshots", "current", "stitch-admin-e2e");
...
for (const target of targets) {
    ...
    console.log(`[ok] captured stitch admin e2e ${target.name}`);
}

await browser.close();
console.log(`[done] stitch admin e2e screenshots saved in ${outDir}`);
