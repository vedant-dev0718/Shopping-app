...
const outDir = path.join(rootDir, "qa", "ui-regression", "screenshots", "current", "stitch-seller-e2e");
...
for (const target of targets) {
    ...
    console.log(`[ok] captured stitch seller e2e ${target.name}`);
}

await browser.close();
console.log(`[done] stitch seller e2e screenshots saved in ${outDir}`);
