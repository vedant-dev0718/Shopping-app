...
FLOW_PATH="$ROOT_DIR/.maestro/flows/admin_e2e_visual_flow.yaml"
APP_SCREENSHOT_DIR="$ROOT_DIR/qa/ui-regression/screenshots/$MODE/admin-e2e"
LOG_DIR="$ROOT_DIR/qa/ui-regression/logs/$MODE/admin-e2e"

if [[ "$MODE" != "current" && "$MODE" != "baseline" ]]; then
  echo "Usage: run-admin-e2e-flow.sh [current|baseline]"
  exit 1
fi
...
echo "[maestro] running admin e2e visual flow ($MODE)"
...
copy_named_screenshot "admin_01_auth_login" "auth_login"
copy_named_screenshot "admin_02_console" "console"

echo "Saved admin e2e screenshots: $APP_SCREENSHOT_DIR"
