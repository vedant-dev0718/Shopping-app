...
class ReelRepository(
    ...
    ): NetworkResult<ReelDto> = runCatchingNetwork { client.get("reels/$id", bearerToken = bearerToken) }
    ...
    suspend fun createReel(
        ...
        bearerToken: String,
    ): NetworkResult<ReelDto> = runCatchingNetwork { client.post("seller/reels", request, bearerToken) }

    suspend fun updateReel(
        id: String,
        request: UpdateReelRequestDto,
        bearerToken: String,
    ): NetworkResult<ReelDto> = runCatchingNetwork { client.patch("seller/reels/$id", request, bearerToken) }

    suspend fun deleteReel(
        id: String,
        bearerToken: String,
    ): NetworkResult<DeleteResponseDto> = runCatchingNetwork { client.delete("seller/reels/$id", bearerToken) }
}
