...
sealed class AppError(
    ...
    data class Server(
        val statusCode: Int,
    ) : AppError("Server error ($statusCode). Try again.")
    ...
    data class Deserialization(
        val detail: String,
    ) : AppError("Unexpected response format: $detail")

    /** An error that doesn't fit the other categories. Avoid using for expected states. */
    data class Unknown(
        val throwable: Throwable,
    ) : AppError(throwable.message ?: "An unexpected error occurred.")
        ...
        fun fromException(e: Exception): AppError {
            ...
            return when {
                ...
                message.contains("no internet") ||
                    ...
                    message.contains("couldn't connect") -> NoNetwork
